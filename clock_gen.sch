v {xschem version=3.4.8RC file_version=1.2}
G {}
K {}
V {}
S {}
F {}
E {}
N 1640 0 1640 20 {lab=V_OUT_1}
N 1640 -30 1640 -10 {lab=V_OUT_1}
N 1640 -60 1750 -60 {lab=VDD}
N 1640 50 1750 50 {lab=VSS}
N 1750 50 1750 280 {lab=VSS}
N 1610 0 1640 0 {lab=V_OUT_1}
N 1640 -10 1640 0 {lab=V_OUT_1}
N 1640 -440 1640 -350 {lab=VDD}
N 1640 -440 1750 -440 {lab=VDD}
N 1750 -320 1750 -60 {lab=VDD}
N 1640 -320 1750 -320 {lab=VDD}
N 1750 -440 1750 -320 {lab=VDD}
N 1640 380 1640 420 {lab=VSS}
N 1750 280 1750 380 {lab=VSS}
N 1640 380 1750 380 {lab=VSS}
N 1640 310 1640 380 {lab=VSS}
N 1640 280 1750 280 {lab=VSS}
N 1540 280 1600 280 {lab=VN_BIAS}
N 1560 -320 1600 -320 {lab=VP_BIAS}
N 1540 210 1540 280 {lab=VN_BIAS}
N 1640 -290 1640 -260 {lab=#net1}
N 1640 -200 1640 -90 {lab=M2_Source}
N 1640 80 1640 130 {lab=#net2}
N 1640 190 1640 250 {lab=#net3}
N 2030 -70 2100 -70 {lab=V_OUT_1}
N 2030 40 2100 40 {lab=V_OUT_1}
N 2140 -20 2140 10 {lab=V_OUT_2}
N 2140 -70 2250 -70 {lab=VDD}
N 2140 40 2250 40 {lab=VSS}
N 2030 -10 2030 40 {lab=V_OUT_1}
N 2030 -70 2030 -10 {lab=V_OUT_1}
N 2140 70 2140 120 {lab=#net4}
N 1640 -10 2030 -10 {lab=V_OUT_1}
N 2140 380 2250 380 {lab=VSS}
N 2140 -440 2250 -440 {lab=VDD}
N 2140 -440 2140 -350 {lab=VDD}
N 2140 -320 2250 -320 {lab=VDD}
N 2250 -440 2250 -320 {lab=VDD}
N 2030 -320 2100 -320 {lab=VP_BIAS}
N 2140 -290 2140 -260 {lab=#net5}
N 2250 -320 2250 -70 {lab=VDD}
N 1750 -440 2140 -440 {lab=VDD}
N 1560 -390 1560 -320 {lab=VP_BIAS}
N 1530 -320 1560 -320 {lab=VP_BIAS}
N 1560 -390 2030 -390 {lab=VP_BIAS}
N 2030 -390 2030 -320 {lab=VP_BIAS}
N 2140 -200 2140 -100 {lab=#net6}
N 2250 40 2250 270 {lab=VSS}
N 2250 270 2250 380 {lab=VSS}
N 2140 300 2140 380 {lab=VSS}
N 2140 270 2250 270 {lab=VSS}
N 2040 270 2100 270 {lab=VN_BIAS}
N 2140 180 2140 240 {lab=#net7}
N 1540 210 2040 210 {lab=VN_BIAS}
N 2040 210 2040 270 {lab=VN_BIAS}
N 1750 380 2140 380 {lab=VSS}
N 3280 -80 3350 -80 {lab=V_OUT_4}
N 3280 30 3350 30 {lab=V_OUT_4}
N 3390 -20 3390 0 {lab=OUT}
N 3390 -80 3500 -80 {lab=VDD}
N 3390 30 3500 30 {lab=VSS}
N 3500 30 3500 380 {lab=VSS}
N 3390 -440 3500 -440 {lab=VDD}
N 3500 -440 3500 -80 {lab=VDD}
N 3390 380 3500 380 {lab=VSS}
N 1530 0 1550 0 {lab=#net8}
N 1530 0 1530 40 {lab=#net8}
N 1580 -10 1580 0 {lab=#net8}
N 1550 -10 1550 0 {lab=#net8}
N 1550 -10 1580 -10 {lab=#net8}
N 1530 40 1580 40 {lab=#net8}
N 1440 0 1450 0 {lab=V_IN_1}
N 1480 -10 1480 0 {lab=V_IN_1}
N 1450 -10 1480 -10 {lab=V_IN_1}
N 1450 -10 1450 0 {lab=V_IN_1}
N 1440 40 1480 40 {lab=V_IN_1}
N 1510 0 1530 0 {lab=#net8}
N 1440 0 1440 40 {lab=V_IN_1}
N 1410 50 1600 50 {lab=V_IN_1}
N 1410 0 1410 50 {lab=V_IN_1}
N 1410 0 1440 0 {lab=V_IN_1}
N 1410 -60 1600 -60 {lab=V_IN_1}
N 1410 -60 1410 0 {lab=V_IN_1}
N 3390 -440 3390 -290 {lab=VDD}
N 3390 -230 3390 -110 {lab=#net9}
N 3390 60 3390 180 {lab=#net10}
N 3390 240 3390 380 {lab=VSS}
N 2420 -70 2490 -70 {lab=V_OUT_2}
N 2420 40 2490 40 {lab=V_OUT_2}
N 2530 -70 2640 -70 {lab=VDD}
N 2530 40 2640 40 {lab=VSS}
N 2530 70 2530 120 {lab=#net11}
N 2530 380 2640 380 {lab=VSS}
N 2530 -440 2640 -440 {lab=VDD}
N 2530 -440 2530 -350 {lab=VDD}
N 2530 -320 2640 -320 {lab=VDD}
N 2640 -440 2640 -320 {lab=VDD}
N 2430 -320 2490 -320 {lab=VP_BIAS}
N 2530 -290 2530 -260 {lab=#net12}
N 2640 -320 2640 -70 {lab=VDD}
N 2250 -440 2530 -440 {lab=VDD}
N 2530 -200 2530 -100 {lab=#net13}
N 2640 40 2640 270 {lab=VSS}
N 2640 270 2640 380 {lab=VSS}
N 2530 300 2530 380 {lab=VSS}
N 2530 270 2640 270 {lab=VSS}
N 2430 270 2490 270 {lab=VN_BIAS}
N 2530 180 2530 240 {lab=#net14}
N 2250 380 2530 380 {lab=VSS}
N 2420 -20 2420 40 {lab=V_OUT_2}
N 2140 -20 2420 -20 {lab=V_OUT_2}
N 2140 -40 2140 -20 {lab=V_OUT_2}
N 2420 -70 2420 -20 {lab=V_OUT_2}
N 2530 -20 2530 10 {lab=V_OUT_3}
N 3280 -20 3280 30 {lab=V_OUT_4}
N 3390 -20 3750 -20 {lab=OUT}
N 3390 -50 3390 -20 {lab=OUT}
N 2430 -390 2430 -320 {lab=VP_BIAS}
N 2040 210 2430 210 {lab=VN_BIAS}
N 2430 210 2430 270 {lab=VN_BIAS}
N 2810 -70 2880 -70 {lab=V_OUT_3}
N 2810 40 2880 40 {lab=V_OUT_3}
N 2920 -70 3030 -70 {lab=VDD}
N 2920 40 3030 40 {lab=VSS}
N 2920 70 2920 120 {lab=#net15}
N 2920 380 3030 380 {lab=VSS}
N 2920 -440 3030 -440 {lab=VDD}
N 2920 -440 2920 -350 {lab=VDD}
N 2920 -320 3030 -320 {lab=VDD}
N 3030 -440 3030 -320 {lab=VDD}
N 2820 -320 2880 -320 {lab=VP_BIAS}
N 2920 -290 2920 -260 {lab=#net16}
N 3030 -320 3030 -70 {lab=VDD}
N 2640 -440 2920 -440 {lab=VDD}
N 2920 -200 2920 -100 {lab=#net17}
N 3030 40 3030 270 {lab=VSS}
N 3030 270 3030 380 {lab=VSS}
N 2920 300 2920 380 {lab=VSS}
N 2920 270 3030 270 {lab=VSS}
N 2820 270 2880 270 {lab=VN_BIAS}
N 2920 180 2920 240 {lab=#net18}
N 2640 380 2920 380 {lab=VSS}
N 2810 -20 2810 40 {lab=V_OUT_3}
N 2530 -20 2810 -20 {lab=V_OUT_3}
N 2810 -70 2810 -20 {lab=V_OUT_3}
N 2920 -20 2920 10 {lab=V_OUT_4}
N 2430 -390 2820 -390 {lab=VP_BIAS}
N 2820 -390 2820 -320 {lab=VP_BIAS}
N 2430 210 2820 210 {lab=VN_BIAS}
N 2820 210 2820 270 {lab=VN_BIAS}
N 2530 -40 2530 -20 {lab=V_OUT_3}
N 2030 -390 2430 -390 {lab=VP_BIAS}
N 2920 -20 3280 -20 {lab=V_OUT_4}
N 2920 -40 2920 -20 {lab=V_OUT_4}
N 3280 -80 3280 -20 {lab=V_OUT_4}
N 3030 -440 3390 -440 {lab=VDD}
N 3030 380 3390 380 {lab=VSS}
N 940 -0 1050 -0 {lab=IN}
N 1100 -0 1410 0 {lab=V_IN_1}
C {sg13g2_pr/sg13_lv_nmos.sym} 1620 50 0 0 {name=M13
l=\{l_p\}
w=\{w_p\}
ng=1
m=1
model=sg13_lv_nmos
spiceprefix=X
}
C {sg13g2_pr/sg13_lv_pmos.sym} 1620 -60 0 0 {name=M14
l=\{l_p\}
w=\{2*\{w_p\}\}
ng=1
m=1
model=sg13_lv_pmos
spiceprefix=X
}
C {lab_wire.sym} 1640 -150 0 0 {name=p9 sig_type=std_logic lab=M2_Source}
C {lab_wire.sym} 1940 -10 0 0 {name=p10 sig_type=std_logic lab=V_OUT_1
}
C {sg13g2_pr/annotate_fet_params.sym} 1490 70 0 0 {name=annot1 ref=M1}
C {sg13g2_pr/sg13_lv_nmos.sym} 1620 280 0 0 {name=M15
l=\{l_p\}
w=\{w_p\}
ng=1
m=1
model=sg13_lv_nmos
spiceprefix=X
}
C {sg13g2_pr/sg13_lv_pmos.sym} 1620 -320 0 0 {name=M16
l=\{l_p\}
w=\{2*\{w_p\}\}
ng=1
m=1
model=sg13_lv_pmos
spiceprefix=X
}
C {ammeter.sym} 1640 -230 0 0 {name=Vmeas_PMOS1 savecurrent=true spice_ignore=0}
C {ammeter.sym} 1640 160 0 0 {name=Vmeas_NMOS1 savecurrent=true spice_ignore=0}
C {capa.sym} 1075 0 3 0 {name=C2
m=1
value=50p
footprint=1206
device="ceramic capacitor"
}
C {sg13g2_pr/sg13_lv_nmos.sym} 2120 40 0 0 {name=M17
l=\{l_p\}
w=\{w_p\}
ng=1
m=1
model=sg13_lv_nmos
spiceprefix=X
}
C {sg13g2_pr/sg13_lv_pmos.sym} 2120 -70 0 0 {name=M18
l=\{l_p\}
w=\{2*\{w_p\}\}
ng=1
m=1
model=sg13_lv_pmos
spiceprefix=X
}
C {lab_wire.sym} 2380 -20 0 0 {name=p12 sig_type=std_logic lab=V_OUT_2}
C {ammeter.sym} 2140 150 0 0 {name=Vmeas_NMOS2 savecurrent=true spice_ignore=0}
C {sg13g2_pr/sg13_lv_pmos.sym} 2120 -320 0 0 {name=M19
l=\{l_p\}
w=\{2*\{w_p\}\}
ng=1
m=1
model=sg13_lv_pmos
spiceprefix=X
}
C {ammeter.sym} 2140 -230 0 0 {name=Vmeas_PMOS2 savecurrent=true spice_ignore=0}
C {sg13g2_pr/sg13_lv_nmos.sym} 2120 270 0 0 {name=M20
l=\{l_p\}
w=\{w_p\}
ng=1
m=1
model=sg13_lv_nmos
spiceprefix=X
}
C {sg13g2_pr/sg13_lv_nmos.sym} 3370 30 0 0 {name=M21
l=\{l_p\}
w=\{w_p\}
ng=1
m=1
model=sg13_lv_nmos
spiceprefix=X
}
C {sg13g2_pr/sg13_lv_pmos.sym} 3370 -80 0 0 {name=M22
l=\{l_p\}
w=\{2*\{w_p\}\}
ng=1
m=1
model=sg13_lv_pmos
spiceprefix=X
}
C {lab_wire.sym} 2780 -20 0 0 {name=p13 sig_type=std_logic lab=V_OUT_3}
C {sg13g2_pr/sg13_lv_pmos.sym} 1580 20 3 0 {name=M23
l=2u
w=0.5u
ng=1
m=1
model=sg13_lv_pmos
spiceprefix=X
}
C {sg13g2_pr/sg13_lv_pmos.sym} 1480 20 3 0 {name=M24
l=2u
w=0.5u
ng=1
m=1
model=sg13_lv_pmos
spiceprefix=X
}
C {ammeter.sym} 3390 -260 0 0 {name=Vmeas_PMOS_OUT savecurrent=true spice_ignore=0}
C {ammeter.sym} 3390 210 0 0 {name=Vmeas_NMOS_OUT savecurrent=true spice_ignore=0}
C {sg13g2_pr/sg13_lv_nmos.sym} 2510 40 0 0 {name=M25
l=\{l_p\}
w=\{w_p\}
ng=1
m=1
model=sg13_lv_nmos
spiceprefix=X
}
C {sg13g2_pr/sg13_lv_pmos.sym} 2510 -70 0 0 {name=M26
l=\{l_p\}
w=\{2*\{w_p\}\}
ng=1
m=1
model=sg13_lv_pmos
spiceprefix=X
}
C {ammeter.sym} 2530 150 0 0 {name=Vmeas_NMOS3 savecurrent=true spice_ignore=0}
C {sg13g2_pr/sg13_lv_pmos.sym} 2510 -320 0 0 {name=M27
l=\{l_p\}
w=\{2*\{w_p\}\}
ng=1
m=1
model=sg13_lv_pmos
spiceprefix=X
}
C {ammeter.sym} 2530 -230 0 0 {name=Vmeas_PMOS3 savecurrent=true spice_ignore=0}
C {sg13g2_pr/sg13_lv_nmos.sym} 2510 270 0 0 {name=M28
l=\{l_p\}
w=\{w_p\}
ng=1
m=1
model=sg13_lv_nmos
spiceprefix=X
}
C {sg13g2_pr/sg13_lv_nmos.sym} 2900 40 0 0 {name=M29
l=\{l_p\}
w=\{w_p\}
ng=1
m=1
model=sg13_lv_nmos
spiceprefix=X
}
C {sg13g2_pr/sg13_lv_pmos.sym} 2900 -70 0 0 {name=M30
l=\{l_p\}
w=\{2*\{w_p\}\}
ng=1
m=1
model=sg13_lv_pmos
spiceprefix=X
}
C {ammeter.sym} 2920 150 0 0 {name=Vmeas_NMOS4 savecurrent=true spice_ignore=0}
C {sg13g2_pr/sg13_lv_pmos.sym} 2900 -320 0 0 {name=M31
l=\{l_p\}
w=\{2*\{w_p\}\}
ng=1
m=1
model=sg13_lv_pmos
spiceprefix=X
}
C {ammeter.sym} 2920 -230 0 0 {name=Vmeas_PMOS4 savecurrent=true spice_ignore=0}
C {sg13g2_pr/sg13_lv_nmos.sym} 2900 270 0 0 {name=M32
l=\{l_p\}
w=\{w_p\}
ng=1
m=1
model=sg13_lv_nmos
spiceprefix=X
}
C {lab_wire.sym} 3200 -20 0 0 {name=p17 sig_type=std_logic lab=V_OUT_4}
C {ipin.sym} 945 0 0 0 {name=p18 lab=IN}
C {iopin.sym} 1640 420 0 0 {name=p19 lab=VSS}
C {opin.sym} 3750 -20 0 0 {name=p20 lab=OUT}
C {iopin.sym} 1640 -440 3 0 {name=p16 lab=VDD}
C {ipin.sym} 1530 -320 0 0 {name=p21 lab=VP_BIAS}
C {ipin.sym} 1540 280 0 0 {name=p22 lab=VN_BIAS}
C {lab_wire.sym} 1270 0 0 0 {name=p1 sig_type=std_logic lab=V_IN_1
}
